...
void kernel_log_hex_u32(const char *label, uint32_t value)
{
    ...
    strcpy(line, label);
    strcpy(line + strlen(line), "0x");
    for (uint8_t i = 0; i < 8; i++) {
        line[strlen(label) + 2 + i] = hex[(value >> ((7 - i) * 4)) & 0xF];
    }
    line[strlen(label) + 10] = '\0';
    log_write(line);
}
...
void kernel_main(void)
{
    ...
    if (!audio_play_startup_chime()) {
        log_write("audio: startup chime skipped");
    }
