...
.PHONY : boot boot_bios boot_install run run_bios run_debug run_install run_uefi run_uefi_debug run_uefi_iso run_uefi_iso_debug run_uefi_q35 run_uefi_q35_debug uefi uefi_iso run_vmware clean default FORCE
...
APP_RZSINSTALL_OBJS = $(APP_RUNTIME_OBJS) out/app_rzsinstall.o
APP_SETUP_OBJS = $(APP_RUNTIME_OBJS) out/app_setup.o
APP_APPDEV_OBJS = $(APP_RUNTIME_OBJS) out/app_appdev.o
APP_MONIDLL_OBJS = out/app_monidll.o
APP_USER_OBJS = $(APP_RUNTIME_OBJS) out/app_demo.o out/app_explorar.o out/app_monilogon.o out/app_player.o out/app_notepad.o out/app_taskmgr.o out/app_square.o out/app_cube3d.o out/app_rzdrv.o out/app_rzsinstall.o out/app_setup.o out/app_appdev.o
APP_PUBLIC_HEADERS = include/syscall.h include/system_status.h user/lib/appsys.h user/lib/stdio.h user/lib/unistd.h
...
KERNEL_UI_OBJS = out/font.o out/graphics.o out/gui.o out/gpu.o out/syscall.o out/app_memory.o out/shell.o out/hash.o out/base64.o out/path.o out/exec.o out/session.o out/browser.o
...
OUT_TARGETS = out/boot.bin out/loader.bin out/kernel.elf out/kernel.bin out/demo.elf out/explorar.exe out/monilog.exe out/player.elf out/notepad.elf out/taskmgr.elf out/square.elf out/cube3d.elf out/rzdrv.elf out/rzdrv.rzs out/rzsinst.elf out/setup.elf out/appdev.elf out/moniapi.dll out/monios.efi out/monios_uefi_esp.img out/monios_uefi_installer.iso $(KERNEL_OBJS) $(APP_USER_OBJS) $(APP_MONIDLL_OBJS) $(UI_FONT_IMAGE) $(UEFI_FONT_IMAGE)
...
out/app_monidll.o : user/apps/monidll.c
	$(X86_64_CC) -c -I include -I user/lib -O0 -fPIC -ffreestanding -fno-builtin -fno-stack-protector -fno-asynchronous-unwind-tables -mno-red-zone -m64 -o out/app_monidll.o user/apps/monidll.c
...
out/rzdrv.rzs : out/rzdrv.elf tools/make_rzs.py
	python tools/make_rzs.py out/rzdrv.elf out/rzdrv.rzs --flags driver,console,r2 --allow-unsigned

out/rzsinst.elf : user/apps/app.ld $(APP_RZSINSTALL_OBJS)
	$(X86_64_LD) -nostdlib -z max-page-size=0x1000 -T user/apps/app.ld -o out/rzsinst.elf $(APP_RZSINSTALL_OBJS)
...
out/appdev.elf : user/apps/app.ld $(APP_APPDEV_OBJS)
	$(X86_64_LD) -nostdlib -z max-page-size=0x1000 -T user/apps/app.ld -o out/appdev.elf $(APP_APPDEV_OBJS)

out/moniapi.dll : $(APP_MONIDLL_OBJS)
	$(X86_64_LD) -shared -nostdlib -z max-page-size=0x1000 -o out/moniapi.dll $(APP_MONIDLL_OBJS)

out/monios.efi : out/kernel.bin tools/build_uefi.py
	python tools/build_uefi.py --output out/monios.efi

out/monios_uefi_installer.iso : out/monios.efi out/kernel.bin out/setup.elf out/boot.bin hd_uefi.img hd.img $(UEFI_FONT_IMAGE) tools/build_uefi_iso.py
	python tools/build_uefi_iso.py --output out/monios_uefi_installer.iso --esp out/monios_uefi_esp.img --font $(UEFI_FONT_IMAGE) --system-uefi hd_uefi.img --system-mbr hd.img --boot-bin out/boot.bin

$(QEMU_INSTALL_DISK) : | $(OUT_DIR)
	qemu-img create -f raw $(QEMU_INSTALL_DISK) 128M

hd_uefi.img : out/boot.bin out/loader.bin out/kernel.bin out/demo.elf out/explorar.exe out/monilog.exe out/player.elf out/notepad.elf out/taskmgr.elf out/square.elf out/cube3d.elf out/rzdrv.rzs out/rzsinst.elf out/setup.elf out/appdev.elf out/moniapi.dll out/monios.efi $(UI_FONT_IMAGE) pwd.txt music_vm.wav bgm.m4a bgm.wav version.txt tools/mkfat32.py
	python tools/mkfat32.py --image hd_uefi.img --boot out/boot.bin --copy out/loader.bin:/loader.bin --copy out/kernel.bin:/kernel.bin --copy out/monios.efi:/EFI/BOOT/BOOTX64.EFI --copy out/monios.efi:/BOOTX64.EFI --copy out/kernel.bin:/KERNEL.BIN --copy out/demo.elf:/apps/demo.elf --copy out/explorar.exe:/apps/explorar.exe --copy out/monilog.exe:/apps/monilog.exe --copy out/player.elf:/apps/player.elf --copy out/notepad.elf:/apps/notepad.elf --copy out/taskmgr.elf:/apps/taskmgr.elf --copy out/square.elf:/apps/square.elf --copy out/cube3d.elf:/apps/cube3d.elf --copy out/rzsinst.elf:/apps/rzsinst.elf --copy out/setup.elf:/apps/setup.elf --copy out/appdev.elf:/apps/appdev.elf --copy out/moniapi.dll:/apps/moniapi.dll --copy out/rzdrv.rzs:/apps/rzdrv.rzs --copy music_vm.wav:/home/root/desktop/music.wav --copy bgm.m4a:/home/root/desktop/bgm.m4a --copy bgm.wav:/home/root/desktop/bgm.wav --copy out/moniapi.dll:/home/root/desktop/moniapi.dll --copy out/rzdrv.rzs:/drivers/rzdrv.rzs --copy out/rzdrv.rzs:/home/root/desktop/rzdrv.rzs --copy $(UI_FONT_IMAGE):/fonts/msyh.ttc --copy pwd.txt:/pwd.txt --copy version.txt:/version.txt
...
hd.img : out/boot.bin out/loader.bin out/kernel.bin out/demo.elf out/explorar.exe out/monilog.exe out/player.elf out/notepad.elf out/taskmgr.elf out/square.elf out/cube3d.elf out/rzdrv.rzs out/rzsinst.elf out/setup.elf out/appdev.elf out/moniapi.dll $(UI_FONT_IMAGE) pwd.txt music_vm.wav bgm.m4a bgm.wav version.txt tools/mkfat32.py
...
run_uefi : hd_uefi.img
	$(QEMU) -machine pc $(QEMU_UEFI_PFLASH) $(QEMU_AUDIO_PCI) -drive file=hd_uefi.img,format=raw $(QEMU_NET) -no-reboot

run_uefi_debug : hd_uefi.img
	$(QEMU) -machine pc -monitor none -serial stdio $(QEMU_UEFI_PFLASH) $(QEMU_AUDIO_PCI) -drive file=hd_uefi.img,format=raw $(QEMU_NET) -no-reboot

run_uefi_iso : out/monios_uefi_installer.iso $(QEMU_INSTALL_DISK)
	$(QEMU) -machine pc $(QEMU_UEFI_PFLASH) -drive file=$(QEMU_INSTALL_DISK),format=raw,if=ide,index=0 -drive file=out/monios_uefi_installer.iso,if=ide,media=cdrom,format=raw,index=1 -boot d $(QEMU_NET) -no-reboot

run_uefi_iso_debug : out/monios_uefi_installer.iso $(QEMU_INSTALL_DISK)
	$(QEMU) -machine pc -monitor none -serial stdio $(QEMU_UEFI_PFLASH) -drive file=$(QEMU_INSTALL_DISK),format=raw,if=ide,index=0 -drive file=out/monios_uefi_installer.iso,if=ide,media=cdrom,format=raw,index=1 -boot d $(QEMU_NET) -no-reboot

run_uefi_q35 : out/monios_uefi_installer.iso $(QEMU_INSTALL_DISK)
	$(QEMU) -machine q35 $(QEMU_UEFI_PFLASH) -drive file=$(QEMU_INSTALL_DISK),format=raw -cdrom out/monios_uefi_installer.iso -boot d $(QEMU_NET) -no-reboot

run_uefi_q35_debug : out/monios_uefi_installer.iso $(QEMU_INSTALL_DISK)
	$(QEMU) -machine q35 -monitor none -serial stdio $(QEMU_UEFI_PFLASH) -drive file=$(QEMU_INSTALL_DISK),format=raw -cdrom out/monios_uefi_installer.iso -boot d $(QEMU_NET) -no-reboot
