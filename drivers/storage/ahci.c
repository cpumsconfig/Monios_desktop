...
static bool ahci_abar_is_identity_mapped(uint32_t abar)
{
    return abar < 0x40000000u || abar >= 0xC0000000u;
}
...
bool ahci_driver_init(void)
{
    ...
    if (!ahci_abar_is_identity_mapped(g_ahci_info.abar)) {
        strcpy(g_ahci_info.status, "ahci: skipped unmapped abar");
        log_write(g_ahci_info.status);
        return true;
    }
