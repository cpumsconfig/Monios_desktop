...
static bool graphics_login_password_ok(const char *password);
...
static void graphics_file_browser_enter_selected(void)
{
    ...
    if (path_len >= 4 && strcmp(path + path_len - 4, ".elf") == 0) {
...
static void graphics_read_pwd_hash(char output[GRAPHICS_LOGIN_PASSWORD_MAX])
{
    int32_t size = file_read("/pwd.txt", output, GRAPHICS_LOGIN_PASSWORD_MAX - 1);
    if (size <= 0) {
        output[0] = '\0';
        return;
    }
    output[size] = '\0';
    while (size > 0 && (output[size - 1] == '\r' || output[size - 1] == '\n' || output[size - 1] == ' ' || output[size - 1] == '\t')) {
        output[--size] = '\0';
    }
}
...
static bool graphics_login_password_ok(const char *password)
{
    char expected[GRAPHICS_LOGIN_PASSWORD_MAX];

    graphics_read_pwd_hash(expected);
    return expected[0] != '\0' && strcmp(password, expected) == 0;
}
...
static const uint8_t (*graphics_find_glyph(uint32_t codepoint))[UI_FONT_WIDTH]
{
    if (codepoint >= UI_FONT_FIRST && codepoint <= UI_FONT_LAST) {
        return g_ui_font[codepoint - UI_FONT_FIRST];
    }
    for (uint32_t i = 0; i < UI_FONT_CJK_COUNT; i++) {
        if (g_ui_font_cjk_codepoints[i] == codepoint) {
            return g_ui_font_cjk[i];
        }
    }
    return NULL;
}

static uint32_t graphics_utf8_next(const char **cursor)
{
...
{
    const uint8_t (*glyph)[UI_FONT_WIDTH];

    glyph = graphics_find_glyph(codepoint);
    if (glyph == NULL) {
        return;
    }
...
{
    while (text != NULL && *text != '\0') {
        uint32_t codepoint = graphics_utf8_next(&text);
        graphics_draw_codepoint(x, y, codepoint, color);
        x = (uint16_t) (x + UI_FONT_WIDTH - 2);
    }
}
...
void graphics_enter_mode(void)
{
    ...
    /* Quick visible test pattern: only perform if kernel page-tables are active. */
    if (mmu_is_active()) {
        for (uint32_t py = 0; py < 100 && py < FB_HEIGHT; py++) {
            for (uint32_t px = 0; px < 100 && px < FB_WIDTH; px++) {
                g_framebuffer[graphics_framebuffer_index((uint16_t)px, (uint16_t)py)] = 0x00FF0000;
            }
        }
    } else {
        log_write("graphics: skipping quick test pattern; mmu not active");
    }
