...
{
    return session_verify_password(password);
}
...
static void shell_change_directory(const char *input)
{
    ...
    strcpy(pwd_pair, "PWD=");
    strcpy(pwd_pair + 4, g_shell_cwd);
    shell_env_set_pair(pwd_pair);
}
    ...
    while (*p != '\0' && oi + 1 < out_size) {
        if (*p == '$') {
            ...
            if (*start == '{') {
                ...
                while (*end != '\0' &&
                       (((uint8_t) *end >= 'A' && (uint8_t) *end <= 'Z') ||
                        ((uint8_t) *end >= 'a' && (uint8_t) *end <= 'z') ||
                        ((uint8_t) *end == '_') ||
                        ((uint8_t) *end >= '0' && (uint8_t) *end <= '9'))) {
                    end++;
                }
                klen = (uint32_t)(end - start);
                p = end;
...
static void shell_run_exec_program(const char *path, uint32_t argc, char *argv[SHELL_ARG_MAX], bool admin_once)
{
    ...
    if ((image_flags & EXEC_IMAGE_FLAG_NEEDS_R2) != 0 ||
        ...
        if (g_shell_privilege != SHELL_PRIV_R0 && g_shell_privilege != SHELL_PRIV_R2 && !admin_once) {
            if (!shell_request_privilege(resolved, "program requests R2", EXEC_PRIV_R2)) {
                shell_print_line("permission denied (need R2)");
                return;
            }
        }
...
static bool shell_request_privilege(const char *program_path, const char *reason, uint32_t privilege_level)
{
    if (g_shell_privilege <= privilege_level) {
        return true;
    }
    if (graphics_active()) {
        return graphics_request_uac_elevation(program_path != NULL ? program_path : "shell", reason, privilege_level);
    }
    return false;
}
...
static void shell_run_command(char *line, bool admin_once)
{
    ...
    uint32_t argc;
    bool allow_admin = (g_shell_privilege <= SHELL_PRIV_R2) || admin_once;
    ...
    if (strcmp(argv[0], "run") == 0) {
        if (argc < 2) {
            shell_print_line("usage: run <program> [args]");
            return;
        }
        shell_run_exec_program(argv[1], argc - 1, &argv[1], admin_once);
        return;
    }
    ...
    if (shell_is_root_only(argv[0]) && !allow_admin) {
        if (!shell_request_privilege(argv[0], "command requests R2", EXEC_PRIV_R2)) {
            shell_print_line("permission denied (need R2)");
            return;
        }
        allow_admin = true;
