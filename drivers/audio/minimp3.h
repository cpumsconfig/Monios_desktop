/*
 * ── MoniOS 集成说明（本地修改） ──────────────────────────────────
 * 这是 minimp3 的 vendored 副本，与 kernel/ui/stb_truetype.h 采用同样的
 * 第三方源码内嵌方式。MPEG-1/2 Audio Layer III 解码器所需的 Huffman 码表、
 * IMDCT 与 32 子带合成滤波器组全部来自这份公有领域实现，不自行手写，
 * 以免引入无法验证的码表错误。
 *
 * 本地改动只有三处，均为适配 freestanding 内核：
 *   1. 去掉 <stdlib.h>（本实现不调用 malloc/free）。
 *   2. <string.h> 改为 "string.h"，解析到 include/string.h（内核版 memcpy/
 *      memset/memcmp）。
 *   3. 默认定义 MINIMP3_NO_SIMD 与 MINIMP3_ONLY_MP3：内核编译单元走标量
 *      路径（不引 immintrin.h），且只保留 Layer III 解码表以控制体积。
 *
 * 真正的入口在 drivers/audio/mp3_dec.c。
 */
...
/* MoniOS: 不需要 <stdlib.h>（无 malloc/free），string.h 走内核版本。 */
#include "string.h"
