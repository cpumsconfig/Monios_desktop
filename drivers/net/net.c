...
bool net_dhcp_request(void)
{
    ...
    for (uint32_t i = 0; i < 400000 && g_dhcp_waiting; i++) {
        net_poll();
        io_wait();
    }
...
bool net_driver_init(void)
{
    ...
    /* If the e1000 MMIO BAR is placed in a very high physical region
     * that the kernel's page tables may not identity-map yet, skip
     * initializing the driver to avoid early page faults. */
    uint32_t mmio_bar = info.bar0 & 0xFFFFFFF0u;
    if (e1000_supported(&info) && mmio_bar != 0 && mmio_bar < 0xF0000000u &&
...
bool net_send_ipv4_packet(const uint8_t dst_ip[4], uint8_t proto, const uint8_t *payload, uint16_t payload_len)
{
    ...
    for (uint32_t i = 0; i < 100000; i++) {
        net_poll();
        if (g_arp_valid && ip_equal(g_arp_ip, next_hop_ip)) {
    ...
    memset(packet, 0, sizeof(packet));
    net_write_ether_header(packet, g_arp_mac, NET_ETH_TYPE_IPV4);
...
bool net_ping(const char *target)
{
    ...
    g_ping_waiting = true;
    if (!net_send_icmp_echo(g_arp_mac, target_ip, g_ping_seq)) {
