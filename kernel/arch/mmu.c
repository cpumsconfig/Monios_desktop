...
static uint64_t pd2[512] __attribute__((section(".pml4"), aligned(4096)));
...
void init_page_tables(void)
{
    ...
    pdpt[2] = (uint64_t) pd2 | PAGE_PRESENT | PAGE_WRITABLE;
    ...
    for (uint64_t i = 0; i < 512; i++) {
        ...
        pd2[i] = ((i + 1024) * 0x200000ULL) | PAGE_PRESENT | PAGE_WRITABLE | PAGE_LARGE_2M;
    }
    pd0[32] |= PAGE_USER;
    pd0[33] |= PAGE_USER;
    for (uint64_t i = 0; i < 512; i++) {
     ...
     * addresses so the entries in the PML4/PDPT are valid physicals. */
    {
        ...
        uint64_t pd2_phys = (kernel_base_va != 0 && (uint64_t)pd2 >= kernel_base_va) ? ((uint64_t)pd2 - kernel_base_va + kernel_base_pa) : (uint64_t)pd2;
        ...
        pdpt[2] = pd2_phys | PAGE_PRESENT | PAGE_WRITABLE;
