...
{
    audio_pc_speaker_tone(880, 1800000);
    audio_busy_delay(350000);
    audio_pc_speaker_tone(1320, 1800000);
    log_write("audio: pc speaker beep");
}
...
bool audio_play_startup_chime(void)
{
    ...
    uint32_t sample_rate = g_audio_device.kind == AUDIO_DEVICE_ES1371 ? es1371_sample_rate() : AC97_TEST_SAMPLE_RATE;
    uint32_t note_frames = sample_rate / 4u;
    uint32_t gap_frames = sample_rate / 15u;
    uint32_t total_frames;

    if (!g_audio_hw_initialized || g_audio_dma == NULL) {
        return false;
    }

    memset(g_audio_dma, 0, AC97_DMA_BYTES);
    audio_fill_tone(g_audio_dma, &frame_cursor, note_frames, sample_rate, 880);
    audio_fill_silence(g_audio_dma, &frame_cursor, gap_frames);
    audio_fill_tone(g_audio_dma, &frame_cursor, note_frames, sample_rate, 1320);

    total_frames = frame_cursor;
    if (total_frames > audio_dma_pcm_capacity_frames()) {
        total_frames = audio_dma_pcm_capacity_frames();
    }
    if (g_audio_device.kind == AUDIO_DEVICE_ES1371) {
        if (!es1371_prepare_pcm_out(&g_audio_device, &g_dma_buffer, total_frames, total_frames, false)) {
            return false;
        }
        strcpy(g_audio_track_name, "ES1371 BEEP");
        g_audio_started = true;
        g_audio_paused = false;
        log_write("audio: playing es1371 startup beep");
        return true;
    }
    if (!audio_ac97_prepare_pcm_out(total_frames, AC97_TEST_SAMPLE_RATE, "AC97 BEEP")) {
        return false;
    }
    log_write("audio: playing startup beep");
    return true;
}
